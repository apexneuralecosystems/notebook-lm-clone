    ...
    try:
        ...
        with st.spinner("Initializing NotebookLM pipeline..."):
            ...
            vector_db = QdrantVectorDB(
                db_path=f"./qdrant_db_{st.session_state.session_id[:8]}", 
            ...
            if assemblyai_key:
                try:
                    ...
                    youtube_transcriber = YouTubeTranscriber(assemblyai_key)
                except ValueError as e:
                    logger.warning(f"Could not initialize audio/YouTube transcribers: {str(e)}")
                    # Continue without audio features
            web_scraper = WebScraper(firecrawl_key) if firecrawl_key else None
            podcast_script_generator = PodcastScriptGenerator(openrouter_key) if openrouter_key else None
            
            # Initialize TTS - completely skip if disabled or if there are any issues
            podcast_tts_generator = None
            enable_tts = os.getenv("ENABLE_TTS", "true").lower() == "true"
            
            if openrouter_key and enable_tts:
                # Quick check if Coqui TTS is importable before attempting initialization
                try:
                    from TTS.api import TTS
                    tts_available = TTS is not None
                except (ImportError, Exception):
                    tts_available = False
                    logger.info("Coqui TTS not available. Skipping TTS initialization.")
                
                if tts_available:
                    try:
                        # Initialize TTS with a short timeout to prevent blocking
                        ...
                        def init_tts():
                            try:
                                tts_result[0] = PodcastTTSGenerator()
                            except Exception as e:
                                tts_error[0] = e
                        
                        # Use a short 20-second timeout to avoid blocking
                        with ThreadPoolExecutor(max_workers=1) as executor:
                            future = executor.submit(init_tts)
                            try:
                                future.result(timeout=20)
                                podcast_tts_generator = tts_result[0]
                                if podcast_tts_generator:
                                    logger.info("PodcastTTSGenerator initialized successfully")
                                elif tts_error[0]:
                                    logger.info(f"Skipping TTS: {tts_error[0]}")
                                    podcast_tts_generator = None
                            except FutureTimeoutError:
                                logger.info("TTS initialization timed out. Skipping TTS.")
                                future.cancel()
                                podcast_tts_generator = None
                            except Exception as e:
                                logger.info(f"Skipping TTS: {e}")
                                podcast_tts_generator = None
                    except Exception as e:
                        logger.info(f"Skipping TTS initialization: {e}")
                        podcast_tts_generator = None
            elif not enable_tts:
                logger.info("TTS is disabled via ENABLE_TTS environment variable.")
            ...
            # Only show info about TTS if it was expected but unavailable
            if podcast_tts_generator is None and openrouter_key and enable_tts:
                st.info("ℹ️ Note: Podcast script generation is available, but audio generation is disabled.")
            
            return True
            
    except Exception as e:
        # TTS errors should already be caught above, so this is for critical failures only
        st.error(f"❌ Failed to initialize pipeline: {str(e)}")
        logger.error(f"Critical pipeline initialization error: {e}", exc_info=True)
        return False
...
def process_youtube_video(youtube_url):
    ...
    with st.spinner("Processing YouTube video..."):
        try:
            ...
            # Handle API key validation errors with helpful message
            error_msg = str(e)
            if "API key" in error_msg:
                st.error(f"❌ {error_msg}")
                st.info("💡 **Tip**: Make sure your `ASSEMBLYAI_API_KEY` is set correctly in your `.env` file. Get your API key from [AssemblyAI](https://www.assemblyai.com/)")
            else:
                st.error(f"❌ {error_msg}")
            logger.error(f"YouTube processing error: {error_msg}")
        except Exception as e:
            error_msg = str(e)
            st.error(f"❌ Failed to process YouTube video: {error_msg}")
            logger.error(f"YouTube processing error: {error_msg}")
...
def process_text(text_content):
    ...
    with st.spinner("Processing text..."):
        try:
            # Use UTF-8 encoding to handle all Unicode characters (fixes Windows charmap error)
...
def generate_podcast(selected_source: str, podcast_style: str, podcast_length: str):
    if not st.session_state.pipeline or not st.session_state.pipeline['podcast_script_generator']:
        st.error("Podcast generation not available. Please check your OpenRouter API key.")
        return
